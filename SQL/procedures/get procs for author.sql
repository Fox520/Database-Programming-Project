...
BEGIN
	SET NOCOUNT ON;
	SELECT AUTHOR.author_id, AUTHOR.first_name, AFFILIATION.affiliation_name, TITLE.title from AUTHOR
			JOIN AFFILIATION ON AUTHOR.affiliation_id = AFFILIATION.affiliation_id
			JOIN TITLE ON AUTHOR.title_id = TITLE.title_id
			WHERE AFFILIATION.affiliation_id = AUTHOR.affiliation_id AND TITLE.title_id = AUTHOR.author_id
